...
public class MapVisualizer : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        ...
        if (data.path != null)
        {
            ...
            {
                Gizmos.DrawCube(data.path[i] + new Vector3(0.5f, 1.5f, 0.5f), new Vector3(0.5f, 0.5f, 0.5f));
    ...
    {
        this.data = data;
        Debug.Log("Punto de Inicio: " + data.startPosition);
        for (int i = 0; i < data.path.Count; i++)
        {
            ...
            for (int row = 0; row < grid.Length; row++)
            {
                ...
                switch (cell.ObjectType)
                {
                    ...
                    case CellObjectType.Obstacle:
                        int randomIndex = UnityEngine.Random.Range(0, environmentTiles.Length);
    ...
    private void CreateIndicator(Vector3 position, GameObject prefab, Quaternion rotation = new Quaternion())
    {
        ...
        element.AddComponent<BoxCollider>(); // Box Collider para el suelo.
        foreach (Transform component in element.GetComponentInChildren<Transform>())
        {
            var c = component.gameObject.AddComponent<MeshCollider>();  // Mesh Collider para cada componente del suelo (árboles, montañas...)
            c.convex = true;
        }
    ...
    private void CreateIndicator(Vector3 position, Color color, PrimitiveType sphere)
    {
        ...
        dictionaryOfObstacles.Add(position, element); // guarda las referencias a los gameobjects del escenario.
