...
def SistemaHormigas(semilla, problema):
    """
    Algoritmo de optimización basado en colonia de hormigas. Lanza una cantidad de hormigas durante cierto tiempo
...
def actualizarFeromonas(distancias, soluciones, feromonas):
    ...
    """
    # Obtenemos el número de hormigas y de arcos del problema
    num_hormigas = len(soluciones)
    num_arcos = len(soluciones[0])

    # Calculamos una matriz para almacenar la cantidad de feromonas que hay que actualizar.
    ...
    for hormiga in range(num_hormigas):
        ...
        # Actualizar delta sumando el valor del coste de cada hormiga en los arcos existentes de la hormiga
        delta[arcos_existentes] += Utils.FACTOR_COSTE / costes_hormigas[hormiga]

    # Aplicar evaporación y actualizar las feromonas
    feromonas *= (1 - Utils.EVAPORACION)  # Evaporación
...
def sembrarFeromonas(feromonas, path):
    """
    Función para sembrar feromonas en un camino específico. Para ello multiplica la cantidad de feromonas que haya en el
    camino seleccionado por un factor de sembrado: FACTOR_SEMBRADO
    Args:
        feromonas: Matriz de feromonas actual.
        path: Camino sobre el que se van a sembrar las feromonas.

    Returns:
        Matriz de feromonas con la cantidad de feromonas del camino indicado por parámetros aumentada en un factor
    FACTOR_SEMBRADO
