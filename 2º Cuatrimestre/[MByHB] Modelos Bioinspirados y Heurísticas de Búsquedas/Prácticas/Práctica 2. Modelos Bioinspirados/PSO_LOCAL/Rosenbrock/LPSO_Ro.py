...
def Local_PSO(semilla, dimensiones):
    ...
    best_valores = np.apply_along_axis(Utils.RosenbrockFunction, 1, pbest)  # Aplica la funcion por filas a la matriz.
    ...
    while t < 1000:
        ...
        for i in range(Utils.NUM_PARTICLES):
            valor = Utils.RosenbrockFunction(x[i, :])
            if valor < Utils.RosenbrockFunction(pbest[i, :]): # Encontrar el mínimo
                pbest[i, :] = x[i, :]
                best_valores[i] = valor

            if Utils.RosenbrockFunction(pbest[i, :]) < mejor_valor:
                mejor_valor = Utils.RosenbrockFunction(pbest[i, :])
                mejor_posicion = x[i, :]
                print(mejor_valor)
        ...
        for i in range(Utils.NUM_PARTICLES):
            ...
            lbest[i, :] = min(np.apply_along_axis(Utils.RosenbrockFunction, 1, x[Utils.GetEntorno(i), :]))
