...
def graficas_resultados(pso):
    for f in [RosenbrockFunction, RastriginFunction]:   # Para cada función
        ...
        for s in range(len(SEEDS)): # Para cada semilla
            ...
            plt.savefig(f"./{'PSO_LOCAL' if pso.__name__ == PSO_LOCAL.Local_PSO.Local_PSO.__name__ else 'PSO_GLOBAL'}/Graficas/{'ROS' if f == RosenbrockFunction else 'RAS'}_S{SEEDS[s]}")
            plt.show()
