		...
		float steer = 0.0f;
			...
			// Mientras no se llegue al estado objetivo
				...
				if(Math.abs(sensors.getTrackPosition()) > 1) {
					...
					/**
					 * Antes de reiniciar deberíamos actualizar la tabla con una recompensa negativa.
					 */
					 ...
					 */
					targetReward = Math.max(sensors.getDistanceRaced()/((Math.abs(sensors.getTrackPosition())*sensors.getAngleToTrackAxis() + 1)),1);
